...
/// EIP-2200/Berlin SSTORE gas calculation
fn calculate_dyn_sstore_gas(
    result: &context_interface::context::SStoreResult,
    is_cold: bool,
) -> u64 {
    ...
    let base_cost = if result.is_new_eq_present() {
        WARM_STORAGE_READ_COST
    } else if result.is_original_eq_present() && result.is_original_zero() {
        SSTORE_SET
    } else if result.is_original_eq_present() {
        WARM_SSTORE_RESET
    } else {
        WARM_STORAGE_READ_COST
    };

    base_cost + if is_cold { COLD_SLOAD_COST } else { 0 }
}

/// CSTORE gas model test for leaking zero vs non-zero info.
#[test]
fn test_cstore_gas_leaks_zero_vs_nonzero_info() {
    let mut db = InMemoryDB::default();
    let address = Address::from_slice(&[0x42; 20]);
    let slot = U256::from(1);

    let mut journal1 = setup_journal_with_account(address, AccountStatus::Created);
    let mut journal2 = setup_journal_with_account(address, AccountStatus::Created);

    // Write non-zero to a zero slot
    let result_zero_to_nonzero = journal1
        .cstore(&mut db, address, slot, U256::from(42), false)
        .unwrap();
    let gas_zero_to_nonzero =
        calculate_dyn_sstore_gas(&result_zero_to_nonzero.data, result_zero_to_nonzero.is_cold);

    // Write non-zero to an already non-zero slot
    let _ = journal2.cstore(&mut db, address, slot, U256::from(100), false);
    let result_nonzero_to_nonzero = journal2
        .cstore(&mut db, address, slot, U256::from(200), false)
        .unwrap();
    let gas_nonzero_to_nonzero = calculate_dyn_sstore_gas(
        &result_nonzero_to_nonzero.data,
        result_nonzero_to_nonzero.is_cold,
    );

    println!(
        "Gas zero->nonzero: {}, nonzero->nonzero: {}",
        gas_zero_to_nonzero, gas_nonzero_to_nonzero
    );

    let gas_leak_exists = gas_zero_to_nonzero != gas_nonzero_to_nonzero;

    assert!(
        gas_leak_exists,
        "Expected gas leak to exist. If fixed, assert gas costs are EQUAL instead."
    );
}
